...
metadata {
	...
	definition (name: "Fire Alarm Button", author: "Carlos Santiago <carloss66@gmail.com>") {
		capability "Switch"
/*        capability "Refresh" */
	}
	...
	tiles {
		standardTile("button", "device.switch", width: 1, height: 1, canChangeIcon: true) {
			state "off", label: 'Fire', action: "switch.on", icon: "st.thermostat.heat", backgroundColor: "#79b821"
			state "on", label: 'Active', action: "switch.off", icon: "st.thermostat.heat", backgroundColor: "#800000"
    }
...
def on() {
    ...
    log.debug "response" : "Request to trigger fire alarm received"
    //log.debug "fire alarm"
